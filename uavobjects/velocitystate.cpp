...
int VelocityState::numInstances = 0;

/**
 * Constructor
 */
VelocityState::VelocityState(UAVObjManager* manager): UAVObject(manager, 0xC243686C, "VelocityState", "Updated by @ref StateEstimationModule, velocity relative to @ref HomeLocation.", "State", 1, 0, ACCESS_READWRITE, ACCESS_READWRITE, 0, 0, UPDATEMODE_PERIODIC, UPDATEMODE_MANUAL, 1000, 0, 0)
{

}

int VelocityState::UAVObjSetDataField(const char *dataIn, const std::string fieldID){
	if(fieldID == "North") {
...
int VelocityState::UAVObjGetDataField(char *dataOut, const std::string fieldID){
	if(fieldID == "North") {
...
int VelocityState::UAVObjUnpack(const unsigned char *dataIn){
	memcpy(&dataFields, dataIn, NUMBYTES);
	return 0;
}

int VelocityState::UAVObjPack(unsigned char *dataOut) {
	memcpy(dataOut, &dataFields, NUMBYTES);
	return 0;
}

unsigned short VelocityState::UAVObjGetInstance() const {
	return instance;
}
unsigned short VelocityState::UAVObjGetNumBytes() const {
	return sizeof(dataFields);
}
unsigned short VelocityState::UAVObjGetNumInstances() const {
	return numInstances;
}

std::string VelocityState::prettyPrint() {
	std::stringstream ss;
	ss << "\nPrinting object: velocitystate" << std::endl;
...
/* */
/*UAVDataObject *VelocityState::clone(unsigned int instID)*/
/*{*/
...
/* */
/*VelocityState *VelocityState::GetInstance(UAVObjectManager *objMngr, unsigned int instID)*/
/*{*/
